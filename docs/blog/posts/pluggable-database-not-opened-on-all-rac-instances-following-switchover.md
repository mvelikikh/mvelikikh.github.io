...
categories:
  - Oracle
date:
  created: 2021-01-16T03:15:00
description: >-
  Pluggable databases in RAC may not open on all instances following Data Guard Switchover.
  The workaround is to create dedicated services for the pluggable databases.
  This post demonstrates the issue and the workaround.
tags:
  - 19c
  - Data Guard
  - OERR
...
Following Data Guard switchover, a pluggable database was not opened on all RAC instances.
This post demonstrates the issue and the fix.
...
[SWITCHOVER](https://docs.oracle.com/en/database/oracle/oracle-database/19/dgbkr/oracle-data-guard-broker-commands.html#GUID-1B802555-FD50-4926-A2B4-8E15B01DCF0B)

> If an Oracle RAC primary database is becoming a physical standby database, all but one instance of the primary database will be shut down before performing the switchover.
> See Switchover for details.

[How the Broker Performs a Switchover](https://docs.oracle.com/en/database/oracle/oracle-database/19/dgbkr/oracle-data-guard-broker-commands.html#GUID-1B802555-FD50-4926-A2B4-8E15B01DCF0B)
...
> If this is an Oracle RAC physical standby database managed by Oracle Clusterware, then the broker directs Oracle Clusterware to restart the new standby database.
...
I tend to think that the current behaviour is not quite consistent.
Either all standby instances should open PDB or neither should open it.
There are several workarounds possible to achieve the desired effect.

Oracle itself recommends using user-defined services:

[Default and User-Defined Services](https://docs.oracle.com/en/database/oracle/oracle-database/19/multi/administering-pdbs-with-sql-plus.html#GUID-5A2471BC-8501-4E38-BD28-7EE182F3B3A7)

> Always use user-defined services for applications.
> The reason is that you can customize user-defined services to fit the requirements of your applications.
> Oracle recommends that you not use the default PDB service for applications.
...
The PDB is opened on all instances this time around.
...
Although the `ORA-65054` error is still there, the pluggable database is opened shortly afterwards by Clusterware.

### racdba1\_ora\_1534.trc
...
User PDB's may not open on all instances following DataGuard Switchover.
This behaviour is quite inconsistent since PDB's are opened just on some instances.
Thankfully, the workaround is pretty straightforward - need to create a dedicated service for these PDB's.
